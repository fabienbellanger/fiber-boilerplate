...
type Handler struct {
	router fiber.Router
	store  repositories.TaskRepository
	logger *zap.Logger
}

// New returns a new Handler
func New(r fiber.Router, task repositories.TaskRepository, logger *zap.Logger) Handler {
	return Handler{
		router: r,
		store:  task,
		logger: logger,
...
func (t *Handler) create() fiber.Handler {
...
func (t *Handler) getAll() fiber.Handler {
	return func(c *fiber.Ctx) error {
		page := c.Query("p")
		limit := c.Query("l")
		sorts := c.Query("s")
...
func (t *Handler) getAllStream() fiber.Handler {
	return func(c *fiber.Ctx) error {
